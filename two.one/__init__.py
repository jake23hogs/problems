...
def test0():
    ...
    check50.run("java Fahrenheit").stdin("0").stdout(number(32.0), "32.0\n").exit(0)
...
def testneg40():
    ...
    check50.run("java Fahrenheit").stdin("-40").stdout(number(-40.0), "-40.0\n").exit(0)

@check50.check(exists)
def test18point5():
    """18.5 degrees Celsius yields 65.3 degrees Fahrenheit"""
    check50.run("java Fahrenheit").stdin("18.5").stdout(number(65.3), "65.3\n").exit(0)

@check50.check(exists)
def testneg123point45678():
    """-123.45678 degrees Celsius yields -190.222204 degrees Fahrenheit"""
    check50.run("java Fahrenheit").stdin("-123.45678").stdout(number(-190.222204), "-190.222204\n").exit(0)

@check50.check(exists)
def test_reject_empty_string():
    """rejects a non-numeric input of "" """
    check50.run("java Fahrenheit").stdin("").reject()
